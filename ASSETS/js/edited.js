...
document.addEventListener("click", function (e) {
  ...
  if (card) {
    ...
    const ing = document.getElementById("popupIngredients");
    const best = document.getElementById("popupBestFor");
    const con = document.getElementById("popupConcerns");

    if (!popup || !img || !title || !desc || !ing || !best || !con) return;
...
// ===== INGREDIENTS =====
const ingUl = document.querySelector("#popupIngredients ul");
ingUl.innerHTML = "";
...
// ===== BEST FOR =====
const bestUl = document.querySelector("#popupBestFor ul");
bestUl.innerHTML = "";
...
// ===== CONCERNS =====
const conUl = document.querySelector("#popupConcerns ul");
conUl.innerHTML = "";
if (Array.isArray(product.concerns)) {
  ...
  /* ===== CLOSE POPUP (BUTTON) ===== */
  if (e.target.classList.contains("product-popup-close")) {
